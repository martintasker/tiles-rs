...
use point2d::{get_point2d_list_from8, get_point2d_list_from12};

fn main() -> std::io::Result<()> {
  let mut writer = SVGWriter::new("out8.svg")?;
  let model8 = model8::get();
  let model8_point2d = model8.iter().map(get_point2d_list_from8).collect();
  writer.write_model(model8_point2d)?;

  writer = SVGWriter::new("out12.svg")?;
  let model12 = model12::get();
  let model12_point2d = model12.iter().map(get_point2d_list_from12).collect();
